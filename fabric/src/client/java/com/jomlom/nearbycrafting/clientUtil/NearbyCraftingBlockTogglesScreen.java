...
public class NearbyCraftingBlockTogglesScreen extends OptionsSubScreen {
    ...
    private static final Method SCROLL_AMOUNT_METHOD = resolveScrollAmountMethod();

    private static Method resolveScrollAmountMethod() {
        for (String name : new String[]{"scrollAmount", "getScrollAmount"}) {
            try {
                return OptionsList.class.getMethod(name);
    ...
    private double currentScrollAmount() {
        if (SCROLL_AMOUNT_METHOD == null) {
            return 0;
        }
        try {
            return (double) SCROLL_AMOUNT_METHOD.invoke(this.list);
    ...
    private void addGroupRows(String namespace, String family, List<String> blockIds) {
        ...
        CycleButton<Boolean> expandToggle = CycleButton.<Boolean>builder(value ->
                        Component.literal((value ? "▼ " : "▶ ") + family + " (" + blockIds.size() + ")"), expanded)
        ...
        CycleButton<Boolean> allToggle = CycleButton.<Boolean>builder(value -> Component.literal("All: ").append(value
                        ...
                        : Component.translatable("nearbycrafting.config.disabled").withStyle(ChatFormatting.RED)), allEnabled)
                .withValues(true, false)
                .displayOnlyValue()
                .create(0, 0, 150, 20, Component.literal(family), (button, value) -> {
                    ...
                    this.minecraft.setScreenAndShow(new NearbyCraftingBlockTogglesScreen(this.lastScreen, new HashSet<>(this.expandedGroups), this.currentScrollAmount()));
                });
    ...
    private void openWithGroup(String groupKey, boolean expanded) {
        ...
        this.minecraft.setScreenAndShow(new NearbyCraftingBlockTogglesScreen(this.lastScreen, next, this.currentScrollAmount()));
    }
    ...
    private void addBlockRow(String namespace, String blockId, String label) {
        ...
        CycleButton<Boolean> toggle = CycleButton.<Boolean>builder(value -> value
                        ? Component.translatable("nearbycrafting.config.enabled").withStyle(ChatFormatting.GREEN)
                        : Component.translatable("nearbycrafting.config.disabled").withStyle(ChatFormatting.RED), enabled)
                .withValues(true, false)
