...
public class NearbyItemsPanel {
    ...
    public static void extract(GuiGraphicsExtractor graphics, Font font, Inventory inventory, int leftPos, int topPos, @Nullable List<Ingredient> requirements) {
        ...
        if (items.size() > SLOTS) {
            graphics.text(font, "...", x + SLOTS_END + ELLIPSIS_OFFSET_X, slotY + ELLIPSIS_OFFSET_Y, TITLE_COLOR, false);
        }
    }
    ...
    private static void drawItem(GuiGraphicsExtractor graphics, Font font, NearbyItem item, int x, int y) {
        graphics.item(item.stack, x, y);
        if (item.count < SCALED_COUNT_FROM) {
            graphics.itemDecorations(font, item.stack, x, y, item.count == 1 ? null : Integer.toString(item.count));
            return;
        }
        graphics.itemDecorations(font, item.stack, x, y, "");
    ...
    private static void drawScaledText(GuiGraphicsExtractor graphics, Font font, Component text, int x, int y, int color, boolean shadow) {
