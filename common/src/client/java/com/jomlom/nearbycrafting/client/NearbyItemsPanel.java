...
public class NearbyItemsPanel {
    ...
    private static final Method LEGACY_DRAW_STRING = resolveLegacyDrawString();

    private static boolean legacyText;
    private static boolean legacyPose;
    ...
    public static void extract(GuiGraphics graphics, Font font, Inventory inventory, int leftPos, int topPos, @Nullable List<Ingredient> requirements) {
        ...
        if (items.size() > SLOTS) {
            drawText(graphics, font, ELLIPSIS, x + SLOTS_END + ELLIPSIS_OFFSET_X, slotY + ELLIPSIS_OFFSET_Y, TITLE_COLOR, false);
        }
    }
    ...
    private static void drawScaledText(GuiGraphics graphics, Font font, Component text, int x, int y, int color, boolean shadow) {
        if (!legacyPose) {
            try {
                ...
                drawText(graphics, font, text, 0, 0, color, shadow);
        ...
        int rightEdge = x + Math.round(font.width(text) * COUNT_SCALE);
        drawText(graphics, font, text, rightEdge - font.width(text), y, color, shadow);
    }

    private static void drawText(GuiGraphics graphics, Font font, Component text, int x, int y, int color, boolean shadow) {
        if (!legacyText) {
            try {
        ...
        try {
            LEGACY_DRAW_STRING.invoke(graphics, font, text, x, y, color, shadow);
        } catch (ReflectiveOperationException e) {
            legacyText = true;
        }
    }

    private static Method resolveLegacyDrawString() {
        Class<?>[] parameters = {Font.class, Component.class, int.class, int.class, int.class, boolean.class};
