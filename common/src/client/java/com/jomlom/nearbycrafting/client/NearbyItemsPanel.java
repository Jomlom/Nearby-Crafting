...
public class NearbyItemsPanel {
    ...
    public static List<Ingredient> requirements(RecipeBookComponent component, RecipeBookMenu<?> menu) {
        RecipeBookPage page = ((RecipeBookComponentAccessor) component).nearbycrafting$getRecipeBookPage();
        RecipeButton hovered = ((RecipeBookPageAccessor) page).nearbycrafting$getHoveredButton();
        if (hovered != null) {
            return ingredients(hovered.getRecipe());
        }
        if (hasPlacedItems(menu)) {
            return ingredients(page.getLastClickedRecipe());
        }
        return null;
    }

    private static List<Ingredient> ingredients(Recipe<?> recipe) {
        return recipe == null ? null : recipe.getIngredients();
    }

    private static boolean hasPlacedItems(RecipeBookMenu<?> menu) {
        for (int i = 0; i < menu.getSize(); i++) {
            if (menu.shouldMoveToInventory(i) && menu.getSlot(i).hasItem()) {
                return true;
            }
        }
        return false;
    ...
    private static List<NearbyItem> visibleItems(Inventory inventory, List<Ingredient> requirements) {
