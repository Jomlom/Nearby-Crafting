...
public class NearbyContainers {
    ...
    private static final Method LEVEL_METHOD = resolveLevelMethod();

    private static Method resolveLevelMethod() {
        for (Method method : Entity.class.getMethods()) {
            if (method.getParameterCount() == 0 && method.getReturnType() == Level.class) {
                return method;
            }
        }
        throw new IllegalStateException("No known Entity level accessor found on this version");
    }

    private static Level levelOf(Player player) {
        try {
            return (Level) LEVEL_METHOD.invoke(player);
